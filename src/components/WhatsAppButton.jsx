...
export default function WhatsAppButton() {
  const href = whatsappLink("Hola, me gustaría hacer una reserva.");

  // Demo: el número de WhatsApp oficial aún no está confirmado.
  // whatsappLink() devuelve null hasta que se rellene site.whatsapp
  // en src/data/site.js — el botón se activa solo en cuanto exista.
  if (!href) {
    return (
      <button
        type="button"
        disabled
        aria-label="WhatsApp — número pendiente de confirmación"
        title="WhatsApp — número pendiente de confirmación"
        className="fixed bottom-5 right-5 z-40 flex h-14 w-14 cursor-not-allowed items-center justify-center rounded-full bg-[#25D366]/40 text-white/80 shadow-lg shadow-black/10 sm:bottom-7 sm:right-7"
      >
